...
description: 第六章 提供了一种识别数据库中表示的实体以及这些实体间如何关联的方式。 最终，数据库设计将会被表示为一个关系数据库设计和一个与之关联的约束集合。
category: 指南
tags: ['DataBase']
---
*上下同欲者胜，风雨同舟者兴*
...
实体一联系数据模型（E-R数据模型）：通过**允许定义代表数据库全局逻辑结构的企业模式（enterprise schema ）**来做到的。
...
* **联系（relationship ）**是**多个实体间**的**相互关联**
* **联系集（relationship set）**是**相同类型联系的集合**。
...
**映射基数（mapping cardinality）** 或基数比率表示一个实体能通过一个联系集关联的另一些实体的数量。

需要注意的是，参与的**基数界限**和**映射基数**是完全相反的

![1766906901172](image/数据库系统概念E-R模型的数据库设计/1766906901172.png)

这里的意思是一个该实体可以映射到 a...b 个对方实体
...
![1766581857254](image/数据库系统概念E-R模型的数据库设计/1766581857254.png)

从具有**复杂属性**的$instructor$实体集的版本转换而来的、**不包含多值属性**的关系模式是这样的：
...
考虑图 6-8 中的E-R图，它描述了包含$phone\_number$多值属性的$instructor$实体集。$instructor$的主码是$ID$。对于这个**多值属性**，**原实体集主键**和该**多值属性**都作为主键

![1766581869403](image/数据库系统概念E-R模型的数据库设计/1766581869403.png)
...
对于从弱实体集转换而来的模式，该模式的主码由其**强实体集的主码**与**弱实体集的主键**组合而成

作为一个示例，考虑图6-15的E-R图中的$section$弱实体集。该实体集有属性：$sec\_id$、$semester$和$year$。$section$所**依赖的$course$实体集的主码**是$course\_id$。因此，我们用具有以下属性的模式来表示$section$：

```
section (course_id, sec_id, semester, year)
```

![1766581766732](image/数据库系统概念E-R模型的数据库设计/1766581766732.png)
...
考虑以下 E/R 图。创建能够体现该 E/R 图的关系模式。对于模式中的每个关系，请指明其主键。

![1766492895950](image/数据库系统概念E-R模型的数据库设计/1766492895950.png)

### 解答

1. 特化继承顶层的主键，因此 company 中包含 empID
2. 特化继承顶层的其他附属性，故 employee 中都有 lot 属性

故各实体集和联系集关系模式如下：

* employee: **empID**, lot
* hourly_employee: **empID**, lot, hours_worked, hours_wages
* contract_employee: **empID**, lot, contract_code
* company: **compID**, name, budget, empID, since_date
* work_in: **empID**, **compID**
