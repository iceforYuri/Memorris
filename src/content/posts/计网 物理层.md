...
$Baud Rate=\frac{1}{T}$ T is the period of a signal unit 波特率等于 1 除以 T，其中 T 表示信号的周期。
...
对于二进制信号, B(Hz)带宽支持的数据率为2B(bps)

实质：

* 只说明带宽时，信道最大数据率：$C=2×B×log_2{V}$
  * **信道最大数据率** ：是指在特定信道条件下，能够可靠传输数据的最高速率。
* 当提及采样率时，信号最大传输率：$C=波特率×log2V=采样率×log_2V$
  * **信号最大传输率** ：信道最大数据率为信号传输设定了一个理论上的上限，而信号最大传输率则是在这个上限范围内，依据信号自身特性和调制方式所能够实现的实际数据传输速率。
* 但无提及V时，理论上C可以无穷大
...
$\frac{signal\_power}{noise\_power}$：信噪比 *signal-to-noise ratio*

但是在使用信噪比之前要先将db转换成无量纲

### SNR (Signal-to-Noise Ratio) **将信噪比的分贝值转换为无量纲数值**
...
##### Nonreturn-to-Zero-Level (NRZ-L)不归零编码

* **信号电平表示** ：**“0” 用低电平来表示，“1” 则用高电平来表示**
* **同步问题** ：NRZ-L 编码存在信号同步方面的缺陷。

##### Nonreturn-to-Zero Inverted (NRZI)反向不归零编码

* **编码规则** ：表示 “1” 时，在间隔开始处有电平跳变（**当“1”时电平翻转**）
* **同步问题** ：与 NRZ-L 类似，NRZI 在遇到连续多个 “0” 时，由于没有电平跳变，接收端难以从信号中准确判断一个符号的结束和下一个符号的起始，容易出现信号同步问题

##### Manchester（曼彻斯特编码）

* **编码规则** ：从高电平跳变到低电平表示 “0”，从低电平跳变到高电平表示 “1”（**高到低0，低到高1**）
...
![1741943435746](image/计网物理层/1741943435746.png)

* 电路交换：
  * 在通信开始前，会建立一条从源端到目的端的物理连接（如图中红色实线所示），这个连接在通话或数据传输期间会一直被占用。
  * 收发共用同一条线路
  * 比如传统的电话系统
* 分组交换：
  * 数据被分割成多个数据包（Packets），每个数据包独立地通过网络进行传输，在传输过程中，不同的数据包可能会经过不同的路径（如图中所示的多个小方块代表数据包），到达目的地后再重新组装。
  * 如互联网
...
* **特点** ：
  ...
  * `Dedicated communication path between 2 stations` 在两个站点之间建立专用的通信路径
  * `Resources allocated in advance` 资源是预先分配好的。
* **三个阶段** ：

  * **电路建立（circuit establishment）** ：在通信开始前，通过网络中的节点（交换机，Node/Switch）建立一条从源端到目的端的物理连接，也就是图中展示的从输入链路（Incoming links）到输出链路（Outgoing links）的连接过程。
  * **数据传输（data transfer）** ：一旦电路建立好，数据就可以通过这条专用路径进行传输。
  * **电路终止（circuit termination）** ：当数据传输完成后，拆除建立的物理连接，释放占用的资源 。

### 优点（Strength）

* **延迟小，质量好（Small delay, good quality）** ：因为电路交换在通信前建立了专用路径，数据传输过程中无需等待其他用户释放资源，所以延迟较小，能保证通信质量。
* **交换机控制简单（Simple control in switches）** ：交换机只需要在连接建立、维持和拆除阶段进行操作，不需要对数据进行复杂的处理，控制逻辑相对简单。

### 缺点（Weakness）

* **建立连接耗时（Time consumed in call set up）** ：在通信前需要建立完整的物理连接，这个过程涉及到多个节点的协调和资源分配，会消耗一定时间。
* **带宽固定，不适合多种速率业务（Fixed bandwidth, not suitable for various rate services）** ：分配的带宽是固定的，当业务实际所需带宽小于分配带宽时，会造成资源浪费；而当业务所需带宽大于分配带宽时，又无法满足需求，因此不适合多种速率的业务场景。
* **双方静默时信道浪费（Channel waste when both parties keep silent）** ：即使通信双方暂时没有数据传输（如通话中的静默期），由于电路是专用的，信道资源也会被持续占用，造成浪费 。

## Packet Switching(分组交换)

* **数据传输形式** ：数据以格式化的比特序列，即数据包（Packets）的形式发送。
* **路由特点** ：每个数据包都是独立路由的。也就是说，在传输过程中，不同的数据包可能会通过不同的路径到达目的地。
* **存储 - 转发机制** ：在网络中的每个节点（Node），整个数据包会被接收、短暂存储，然后转发到下一个节点，这就是存储 - 转发（Store - and - Forward）机制。受通畅程度影响
* **资源分配方式** ：采用动态资源分配，根据网络当前的状况和数据包的需求，灵活地分配网络资源，提高资源利用率。

### 基本操作

#### 数据传输方式 Data transmitted in small packets

* circuit switching：数据以小数据包的形式传输，通常每个数据包大小为 1000 字节（octets，即 bytes）。
* Packet switching： `Longer messages split into series of packets  `较长的信息会被拆分成一系列的数据包。
* 每个数据包不仅包含一部分用户数据，还包含一些控制信息。

#### 控制信息内容 Control information

* `Routing (addressing) information  `控制信息主要包含路由（寻址）信息，用于指导数据包在网络中选择合适的路径，以确保数据包能准确到达目的地。

# Physical layer protocol: 10BaseT

## Physical Interface Features 物理接口特性

* **机械特性（Mechanical features）** ：主要明确接口所用接线器的形状、尺寸、引线数目与排列方式，以及固定和锁定装置等内容。
* **电气特性（Electrical features）** ：规定了在接口电缆各条线上出现的电压范围
  * 还涉及编码方式、数据传输速率以及电缆长度等方面。
* **功能特性（Functional features）** ：指明某条线上出现的某一电平的电压所代表的意义。
* **过程特性（Procedure features）** ：确定了对于不同功能的各种可能事件的出现顺序，也就是通信过程中各个操作的先后顺序。

## Physical Interface: Ethernet 以太网（Ethernet）的物理接口特性

* **标准和类型** ：遵循 IEEE802.3 标准，具体类型为 10BaseT。
* **数据传输速率和介质** ：数据传输速率是 10Mbps，使用双绞线作为传输介质，成本低且便于安装。
* **机械特性** ：采用 RJ - 45 接口，有 8 根引脚，这种接口常见且标准化，便于设备连接。
* **电气特性** ：采用曼彻斯特编码，能将时钟和数据包含在同一信号中。信号电压为 2.5v 和 -2.5v，保证信号传输的准确性。
* **功能特性** ：有 1 对引脚（pin 1&2）用于发送数据，1 对引脚（pin 3&6）用于接收数据，支持全双工通信，即数据能同时双向传输，提高传输效率。

# Key Points of Physical Layer

- **概念区分**：
  - **数据与信号（Data vs. Signal）**：数据是信息的载体，信号是数据的电磁表示形式，用于数据传输。
  - **模拟与数字（Analog vs. Digital）**：模拟是连续变化的量，数字是离散的量。在通信中，模拟信号可通过调制解调转换为数字信号，反之亦然。
  - **带宽与信道容量（Bandwidth vs. Channel capacity）**：带宽指信号具有的频带宽度或传输介质能传输的频率范围；信道容量是信道无差错传输的最大数据速率。
  - **异步与同步（Asynchronous vs. Synchronous）**：异步通信中，字符间时间间隔不固定，靠附加位实现同步；同步通信中，收发双方有共同时钟，数据以块传输，通过时钟信号保持同步。
  - **波特率与比特率（Baud rate vs. Bit rate）**：波特率是单位时间内信号变化的次数；比特率是单位时间内传输的二进制位数，两者关系与编码方式有关。
  - **传输延迟与传播延迟（Transmission delay vs. Propagation delay）**：传输延迟是发送设备发送数据帧所需时间，与数据帧长度和发送速率有关；传播延迟是信号在传输介质中传播所需时间，与介质长度和信号传播速度有关。
- **重要定理**：
  - **奈奎斯特定理（Nyquist theorem）**：在无噪声信道中，给出了码元传输速率上限，即 $C = 2W log_2V$ （C为信道容量，W为带宽，V为信号电平数）。
  - **香农定理（Shannon theorem）**：在有噪声信道中，给出了信道容量的极限值，即 $C = Wlog_2(1+S/N)$ （C为信道容量，W为带宽，S/N为信噪比）。
- **传输介质（Transmission media）**：包括有线介质（如双绞线、同轴电缆、光纤）和无线介质（如无线电波、微波、红外线）。
- **调制原理（Principles of modulation）**：
  - **四相移键控（QPSK）**：利用载波的四种不同相位来表示数字信号，每个码元携带2比特信息。
  - **正交幅度调制（QAM）**：结合载波的幅度和相位变化来传输数据，可在相同带宽下传输更多数据。
- **线路编码（Line encoding）**：以曼彻斯特编码为例，将时钟和数据包含在同一信号中，便于接收端提取时钟信号，常用于以太网。
- **技术概念（Idea of）**：
  - **非对称数字用户线路（ADSL）**：利用电话线实现高速数据传输，下行速率高于上行速率。
  - **脉冲编码调制（PCM）**：将模拟信号转换为数字信号的过程，包括采样、量化和编码三个步骤。
- **多路复用技术（Multiplexing）**：
  - **频分多路复用（FDM）**：将不同信号调制到不同频率段，在同一传输介质上同时传输。
  - **波分多路复用（WDM）**：用于光纤通信，将不同波长的光信号复合在一根光纤中传输。
  - **时分多路复用（TDM）**：将时间划分为时隙，不同信号在不同时隙传输。
  - **码分多址（CDMA）**：每个用户被分配一个唯一的码序列，利用码序列的正交性在同一时间、同一频段传输多个信号。
- **交换技术对比**：
  - **电路交换（Circuit Switching）**：通信前建立专用物理连接，通信中独占链路，通信结束拆除连接。
  - **分组交换（Packet Switching）**：数据分割成数据包，独立路由传输，到达后重组，资源动态分配。
