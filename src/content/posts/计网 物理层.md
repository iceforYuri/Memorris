...
$\frac{signal\_power}{noise\_power}$：信噪比 *signal-to-noise*
...
* **存储 - 转发机制** ：在网络中的每个节点（Node），整个数据包会被接收、短暂存储，然后转发到下一个节点，这就是存储 - 转发（Store - and - Forward）机制。
...
* 数据以小数据包的形式传输，通常每个数据包大小为 1000 字节（octets，即 bytes）。
* `Longer messages split into series of packets  `较长的信息会被拆分成一系列的数据包。
