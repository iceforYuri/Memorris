...
曼彻斯特编码是时钟信号与 NRZ 信号异或（XOR）的结果，还给出了异或运算规则（X⊕0=X，X⊕1=X̅ ）。
...
* `The bandwidth efficiency of Manchester is 50%` 曼彻斯特编码用 2 个符号传 1 比特，所以其带宽效率只有 50%。
...
- **统计时分复用（Statistical TDM）**：根据用户实际有无数据传输来动态分配时隙。只有当用户有数据（如 $A_1、B_1、B_2、C_2$）时，才会分配时隙，这样能有效利用信道资源，减少空闲时隙。但每个数据单元需要添加地址信息（蓝色方块），以便接收端能正确区分数据来自哪个用户。图中也展示了在某些周期中存在未使用的容量（Unused capacity），不过相比同步时分复用，整体资源利用率更高 。
