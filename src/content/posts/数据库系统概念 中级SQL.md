...
自然连接只考虑在两个关系的模式中都出现的那些属性上取值相同的元组对，并会去除重复的属性（连接那一个判断）
...
外连接（outer join）运算通过在结果中创建包含空值的元组，来保留那些在连接中会丢失的元组。
...
当join 子句中没有使用 outer 前缀时，缺省的连接类型是内连接。
...
* 默认：当违反引用完整性约束时，通常的处理是拒绝执行导致破坏完整性的操作（即执行更新操作的事务被回滚）
